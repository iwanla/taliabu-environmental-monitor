...
- Alert log block di bawah AOI: daftar alert tersimpan (dot severity, kind label, ha, tanggal), dropdown filter by kind, klik entri → `focusBounds` prop → `map.fitBounds` ke bbox evidence (maxZoom 13).
